...
import { DropdownContextType } from "./_context";

interface Props<T> {
  getContext: () => DropdownContextType<T>;
  label: string;
  renderItem: React.ComponentProps<typeof ListDropdown>["renderItem"]
}

export function DropdownIndex<T>({
  label,
  renderItem,
  getContext
}: Props<T>) {
    ...
    <View>
      ...
      >
        <ModalBox>
          ...
          <ListDropdown {...{ renderItem, context }} />
          <DismissButton />
        </ModalBox>
      </Modal>
    </View>
