...
export default function Home({ navigation }: PaymentsProps) {
  const theme = useTheme();

  const [paymentsInstallment, setReceiptsInstallment] = useState<Installment[]>([]);
  ...
  useEffect(() => {
    ...
    PaymentInstallmentApi.list_all.execute().then((paymentsInstallment) => {
      setReceiptsInstallment(paymentsInstallment);
    }).catch((error) => {
  ...
  return (
    <BaseView>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={{ textAlign: "center", marginTop: 20 }}>Nenhum pagamento parcelado encontrado.</Text>}
