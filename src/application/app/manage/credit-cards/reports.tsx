...
export interface CreditCardsReportsParams extends UnknownOutputParams {
	id: string;
	name: string;
	limit: string;
}

export default function CreditCardsReports() {
	const { id, name, limit } = useLocalSearchParams<CreditCardsReportsParams>();
	...
	return (
		<BaseView style={{ paddingHorizontal: 0 }}>
			<TitlePage>
				Relatórios da conta no <TextBold>{name}</TextBold>
			</TitlePage>
			<FlatList
