...
export const base_item_value = sqliteTable("base_item_value", {
  ...
  transfer_method_type_id: t.integer("fk_id_transfer_method_type").references(() => transfer_method_type.id).notNull(),
  tag_id: t.integer("fk_id_tag").references(() => tag.id).notNull(),
...
export const base_item_value_relations = relations(base_item_value, ({one}) => ({
  transfer_method_type: one(transfer_method_type, {
    fields: [base_item_value.transfer_method_type_id],
    references: [transfer_method_type.id]
  }),
  tag: one(tag, {
    fields: [base_item_value.tag_id],
    references: [tag.id]
  }),
}))