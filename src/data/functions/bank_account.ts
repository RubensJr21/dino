...
export async function get_by_nickname(db: DatabaseType, bank_nickname: DataSelect["nickname"]) {
  return await db.query.bankAccount.findMany({ where: eq(bankAccount.nickname, bank_nickname) })
}
