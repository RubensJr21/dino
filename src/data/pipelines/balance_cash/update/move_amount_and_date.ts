...
interface MoveAmountAndDateParams {
...
export async function move_amount_and_date({
  new_date, old_date,
  new_amount, old_amount,
  cashflow_type,
  was_processed
}: MoveAmountAndDateParams) {
  ...
  {
    const oldRealAmountValue = getRealAmountValue(cashflow_type, old_amount, true)

    const planned_amount = balance_cash_out.planned_amount + oldRealAmountValue
    const executed_amount = was_processed ? balance_cash_out.executed_amount + oldRealAmountValue : undefined
  ...
  const newRealAmountValue = getRealAmountValue(cashflow_type, new_amount)

  if (balance_cash_in === undefined) {
    await bc.create_balance(db, {
      amount: newRealAmountValue,
      month: new_date.getMonth(),
      year: new_date.getFullYear()
    })
    return;
  ...
  {
    const planned_amount = balance_cash_in.planned_amount + newRealAmountValue
    const executed_amount = was_processed ? balance_cash_in.executed_amount + newRealAmountValue : undefined
