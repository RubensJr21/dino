...
interface RegisterRecurringReceiptInput extends StrictOmit<DTO_RecurringItemValue, "base_item_value"|"recurrence_type"> {
    base_item_value: BaseItemValue_RegisterInput;
    recurrence_type: string;
}

export default class RegisterRecurringReceipt implements IUseCase<RegisterRecurringReceiptInput, RecurringItemValue> {
    constructor(
    ...
    async execute(input: RegisterRecurringReceiptInput): Promise<RecurringItemValue> {
        ...
        if (!recurring_item_value) {
            throw new Error("Error creating Recurring Receipt!")
        } else {
