...
interface UnmarkRecurringReceiptAsProcessed_Input {
    id: number
}

export default class UnmarkRecurringReceiptAsProcessed implements IUseCase<UnmarkRecurringReceiptAsProcessed_Input, RecurringItemValue>{
    constructor(
        private repo_biv: IRepoBaseItemValue,
        private repo_iv: IRepoRecurringItemValue
    ){}
    async execute(input: UnmarkRecurringReceiptAsProcessed_Input): Promise<RecurringItemValue> {
