...
import { DTO_BaseItemValue, DTO_RecurringItemValue } from "@core/shared/DTOTypes";
...
interface BaseItemValue_RegisterInput extends StrictOmit<DTO_BaseItemValue, "tag"|"transfer_method_type"|"type"> {
    tag: string;
    transfer_method_type: string;
}

interface RegisterRecurringPaymentInput extends StrictOmit<DTO_RecurringItemValue, "base_item_value"|"recurrence_type"> {
    base_item_value: BaseItemValue_RegisterInput;
    recurrence_type: string;
}

export default class RegisterRecurringPayment implements IUseCase<RegisterRecurringPaymentInput, RecurringItemValue> {
    constructor(
    ...
    async execute(input: RegisterRecurringPaymentInput): Promise<RecurringItemValue> {
        ...
        if (!recurring_item_value) {
            throw new Error("Invalid Recurring Payment returned!")
        } else {
