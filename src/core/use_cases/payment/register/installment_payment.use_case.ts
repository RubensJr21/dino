...
interface RegisterInstallmentPaymentInput extends StrictOmit<DTO_InstallmentItemValue, "base_item_value"|"installments_number"> {
    base_item_value: BaseItemValue_RegisterInput;
    installments_number: number;
}

export default class RegisterInstallmentPayment implements IUseCase<RegisterInstallmentPaymentInput, InstallmentItemValue> {
    constructor(
    ...
    async execute(input: RegisterInstallmentPaymentInput): Promise<InstallmentItemValue> {
        ...
        if (!installment_item_value) {
            throw new Error("Invalid Installment Payment returned!")
        }
