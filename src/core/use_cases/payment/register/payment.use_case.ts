...
import { DTO_BaseItemValue, DTO_ItemValue } from "@core/shared/DTOTypes";
...
interface RegisterPaymentInput extends StrictOmit<DTO_ItemValue, "base_item_value"> {
    base_item_value: BaseItemValue_RegisterInput
}

export default class RegisterPayment implements IUseCase<RegisterPaymentInput, ItemValue> {
    constructor(
    ...
    async execute(input: RegisterPaymentInput): Promise<ItemValue> {
        ...
        if (!item_value) {
            throw new Error("Invalid Payment returned!")
        } else {
            return item_value;
        }
    }
